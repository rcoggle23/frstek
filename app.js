...
client.on("interactionCreate", async interaction => {
  ...
  if (!TicketSetupData)
    return interaction.reply({ content: `Bu sistem için veriler eski`, ephemeral: true, fetchReply: true });
  ...
  db.findOne({ ChannelID: channel.id }, async (err, docs) => {
    ...
    switch (customId) {
      case "sil":
        interaction.reply({ content: "Kanal siliniyor", ephemeral: false, fetchReply: true });
      ...
      case "lock":
       ...
       // channel.setName(`kapalı-${docs.TicketID}`)
      ...
      case "unlock":
        if (docs.Locked == false) return interaction.reply({ content: "Bu destek talebi zaten açık", ephemeral: true, fetchReply: true })
          .catch(err => interaction.channel.send({ content: `${err}`, ephemeral: true }));
        await db.updateOne({ ChannelID: channel.id }, { Locked: false })
        //channel.setName(`destek-${docs.TicketID}`)
      ...
      case "close":
        ...
        if (docs.Closed) {
          ...
          }).catch(err => interaction.channel.send({ content: `${err}` }));
          return;
        }
        //channel.setName(`siliniyor`)
        ...
        await guild.channels.cache.get(Data.Transcripts).send({
          embeds: [Embed.setTitle("Talep Kapatıldı")
          .setDescription(
            `Talebi Açan Kullanıcı ID:\`${docs.AuthorId}\`
            Talebi Kapatan ID:\`${member.id}\`
            Talep ID: ${docs.TicketID}
            Açılış Tarihi: <t:${parseInt(docs.CreatedAt / 1000)}:R>
            Kapanış Tarihi: <t:${parseInt(new Date() / 1000)}:R>
            `
            )],
          files: [dosya],
        });
        interaction.reply({ content: `Destek talebi Kaydedildi ve kapatıldı!\nBu kanal 5 saniye sonra silinecektir`, fetchReply: true })
          .catch(err => interaction.channel.send({ content: `${err}`, ephemeral: true }));
